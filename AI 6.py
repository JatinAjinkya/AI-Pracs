...
def a_star(graph, prev, dst, path, pcost, q):
    ...
    print("Connected nodes of current node", prev, "with h(n) values: ")    #Arad
    for n in graph[prev][0]:    #neighbors list n =Z, S, T
        if n not in path:
            q[n] = (graph[n][1], graph[prev][0][n])
            print(n, "->", q[n])
            add1=sum(q[n])
            path_cost=pcost+add1        # A to C = A to B?? B to C ?? tot A* value =prev path +(h+g)
...
#print(" -> ".join(a_star(graph, "Arad", "Vaslui", [], 0, {"Arad": (399, 0)})) or "Path not found")
