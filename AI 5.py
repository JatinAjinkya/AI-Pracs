...
def greedy_search_rec(graph, prev, dst, path, q):
    ...
    for n in graph[prev][0]:    #neighbors list prev=arad, -> Z,S,T
        if n not in path:
...
source=input("Enter source vertext: ")
dest=input("Enter destination vertext: ")
...
#print("Greedy Search Recursive:", " -> ".join(greedy_search_rec(graph, "Pitesti", "Bucharest", ["Pitesti"], {})) or "Path not found")    
